...
class Broker{
	...
	private static function init_foreign_keys($table_name, $_meta)
	{
		...
		if ($_meta->has_foreign_keys())
		{
			foreach ($_meta->db_foreign_keys() as $key => $value) 
			{
				...
				Table_Manager::add_foreign_keys($table_name, $key, $foreign_meta->table_name(), $value,'no action', 'no action');
			}
		}
	}
	...
	public static function init($obj)
	{
		...
		Broker::init_foreign_keys($table_name, $_meta);
	...
	public static function save($obj, $meta)
	{
		...
		$is_update = false;
		$is_update = Broker::id_exist($obj_class, $obj->get_primary_key_value());
